...
internal class Container(private val lazy: Boolean, private val environment: Environment) {
    ...
    fun <T : Any> newProperty(name: String, config: PropertyConfig<T>): PropertyImpl<T> =
        PropertyDefinition(environment.profiles).let { definition ->
            PropertyImpl(
                PropertyDelegate(
                    name = name,
                    ...
                    instance = {
                        ...
                        config.instance(definition, value)
                    }
                )
            )
        }.apply(::register)
    ...
    fun <T : Any> newBean(name: String, config: BeanConfig<T>): BeanImpl<T> =
        BeanDefinition(environment.profiles).let { definition ->
            BeanImpl(
                BeanDelegate(
                    name = name,
                    type = config.type,
